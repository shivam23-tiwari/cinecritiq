...
import { collection, query, getDocs, doc, getDoc } from '../lib/firestore-wrapper';
...
export default function Community() {
        ...
        for (const userDoc of snap.docs) {
          ...
          try {
            ...
            actionsSnap.docs.forEach(actionDoc => {
              ...
              const movieId = actionData.movieId || actionData.id;
          ...
          // Favorites 4 logic: top 4 highest rated movies
          const top4 = Array.from(rateMap.values())
          ...
          fetchedUsers.push({
            ...
            favoriteMovies: top4,
            rawCustomPosters: customPosters
          });
        }
  ...
  const UserCard = ({ u }: { u: any }) => (
    <div className="flex flex-col items-center group">
      <Link to={\`/user/\${u.id}\`} className="mb-3 relative">
        <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden bg-[#2c3440] border-4 border-[#1a1f26] group-hover:border-[#38bdf8] transition-colors shadow-lg">
          <img 
      ...
      <Link to={\`/user/\${u.id}\`} className="flex items-center justify-center gap-1.5 w-full hover:text-[#40bcf4] transition-colors mb-0.5">
        <span className="text-white font-bold text-[14px] md:text-[15px] truncate max-w-full">{u.displayName || u.email?.split('@')[0] || 'Member'}</span>
        {isGoat(u) && (
      ...
      <p className="text-[11px] md:text-[12px] text-[#89a] mb-3">
        {u.stats?.watched || 0} films • {u.stats?.rated || 0} reviews
      </p>
      
      {u.commonCount > 0 && filterSameLikeYou && (
        <div className="mb-2 px-2 py-0.5 rounded bg-[#00e054]/20 text-[#00e054] text-[10px] font-bold">
          {u.commonCount} in common
        </div>
      )}

      <div className="flex gap-0.5 w-full max-w-[140px] h-[52px] md:h-[68px] justify-center">
        {[0, 1, 2, 3].map(i => {
          const movie = u.favoriteMovies && u.favoriteMovies[i];
          const hasImage = movie && movie.posterPath && !movie.posterPath.includes('undefined');
          return (
            <div key={i} className="flex-1 bg-[#2c3440] rounded-[2px] overflow-hidden shadow-sm border border-white/5 relative">
              {hasImage ? (
                <img 
  ...
  if (loading) {
    return (
      <div className="pt-24 pb-20 px-4 max-w-5xl mx-auto min-h-screen flex justify-center items-center">
