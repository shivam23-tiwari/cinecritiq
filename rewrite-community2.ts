    ...
    const fetchUsers = async () => {
      try {
        ...
        for (const userDoc of snap.docs) {
          ...
          const favoriteMoviesMap = new Map();
          ...
          try {
            ...
            actionsSnap.docs.forEach(actionDoc => {
              ...
              if (actionData.actionType === 'favorite') {
                favoriteMoviesMap.set(movieId, {
                   id: movieId,
                   posterPath: actionData.movieData?.poster_path || actionData.poster_path,
                   createdAt: actionData.createdAt?.toMillis ? actionData.createdAt.toMillis() : 0
                });
              }
            });
            ...
            if (legacyRatings && legacyRatings.docs) {
               ratedCount += legacyRatings.size;
               watchedCount += legacyRatings.size;
            }
            
            const legacyFavs = await getDocs(query(collection(db, \`users/\${userId}/favorites\`))).catch(() => null);
            if (legacyFavs && legacyFavs.docs) {
               legacyFavs.docs.forEach(d => {
                  const data = d.data();
                  const movieId = data.id || data.movieId;
                  if (!favoriteMoviesMap.has(movieId)) {
                    favoriteMoviesMap.set(movieId, {
                      id: movieId,
          ...
          // Favorites 4 logic: top 4 favorite movies (by recent, or just take first 4)
          const favoriteMovies = Array.from(favoriteMoviesMap.values())
      ...
      {u.favoriteMovies && u.favoriteMovies.length > 0 && (
        <div className="flex gap-1 w-full mt-auto">
          {[0, 1, 2, 3].map(i => {
            ...
            return (
              <div key={i} className="flex-1 aspect-[2/3] bg-[#2c3440] rounded-[3px] overflow-hidden shadow-sm border border-white/5 relative">
                {hasImage ? (
                  <img 
      ...
      <div>
        ...
        {filterSameLikeYou && currentUserFavorites.length === 0 && (
          <div className="mb-6 p-4 bg-[#2c3440]/50 border border-white/10 rounded-lg text-[#89a] text-sm text-center">
            You need to add at least 1 movie to your favorites to find matches!
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
          ...
          {displayedUsers.length === 0 && (
            <div className="col-span-full py-12 text-center text-[#89a]">
              {filterSameLikeYou ? "No members found with similar favorite movies." : "No members found matching your search."}
            </div>
          )}
        </div>
      </div>
